...
class Client:
    ...
    def find_people(self, skills):
        ...
        url = (
            f"https://www.linkedin.com/search/results/people/?keywords={encoded_string}"
        )
        ...
        for person in people:
            try:
                ...
                result["name"] = person.find_element(
                    By.CSS_SELECTOR, "span.entity-result__title-line"
                ).text
                result["position"] = person.find_element(
                    By.CSS_SELECTOR, "div.entity-result__primary-subtitle"
                ).text
                result["location"] = person.find_element(
