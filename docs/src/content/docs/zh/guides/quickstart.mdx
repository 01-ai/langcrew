...
from langcrew.tools import SearchTool, WebScrapeTool
```
...
# Hierarchical execution (with a manager agent)
crew = Crew(
...
### Review and Revision
```python
creator → reviewer → creator (revision)
```

### Parallel Specialists
```python
specialist1 ↘
specialist2 → aggregator → final output
specialist3 ↗
```
