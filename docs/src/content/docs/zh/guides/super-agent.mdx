...
- **云手机自动化配置**：用于移动设备自动化操作
  - `AGENTBOX_API_KEY`：AgentBox 云手机服务 API 密钥
  - `AGENTBOX_TEMPLATE`：云手机模版
...
#### 云手机自动化
- 基于 AgentBox 的智能移动设备操作和应用自动化
- 支持 Android 设备的完整 UI 交互和应用管理
- 智能截屏和可点击元素识别，提供实时视觉上下文
- 全面的移动端操作能力：点击、滑动、文本输入、按键操作
- 应用生命周期管理：启动应用、切换应用、获取应用列表
- 支持复杂移动端任务：购物、社交、搜索、内容浏览等
- 智能错误处理和任务恢复机制
- 支持用户接管控制处理验证码、登录等敏感操作
...
curl -X POST "http://localhost:8000/api/v1/chat" \
  -H "Content-Type: application/json" \
  -d '{
    "message": "启动淘宝应用，搜索iPhone 15 Pro，查看前三个商品的价格并截图保存",
...
│   └── prompt.py           # 系统提示词
├── tool/
│   └── cloud_phone_streaming_tool.py           # 自定义工具
├── main.py                 # 直接执行入口点
