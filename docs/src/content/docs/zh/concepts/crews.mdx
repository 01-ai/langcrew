...
tasks = [Task(agent=agents[0], description="研究AI趋势"), Task(agent=agents[1], description="撰写报告")]
...
research_task = Task(agent=researcher, description="研究市场趋势", name="research")
analysis_task = Task(agent=analyst, description="分析发现", context=[research_task], name="analysis")
report_task = Task(agent=writer, description="写最终报告", context=[analysis_task], name="report")
...
crew = Crew(
    ...
    tasks=[critical_task],
    hitl=HITLConfig(enabled=True)  # 需要人工批准
)
```
...
crew = Crew(
    agents=[
        Agent(role="研究员", goal="收集数据"),
        Agent(role="分析师", goal="分析发现"), 
        Agent(role="写手", goal="创建报告")
    ],
    tasks=[
        Task(agent=researcher, description="研究主题", name="research"),
        Task(agent=analyst, description="分析数据", context=["research"], name="analyze"),
        Task(agent=writer, description="写报告", context=["analyze"], name="report")
    ]
)
```
...
crew = Crew(
    agents=[
        Agent(role="作者", goal="创建内容"),
        Agent(role="审核员", goal="审核质量"),
        Agent(role="批准者", goal="最终批准")
    ],
    tasks=[
        Task(agent=author, description="创建草稿", name="draft"),
        Task(agent=reviewer, description="审核内容", context=["draft"], name="review"),
        Task(agent=approver, description="最终批准", context=["review"], name="approve")
    ]
)
```
...
继续学习 [内存管理](/concepts/memory) - 理解有状态工作流的持久记忆系统。