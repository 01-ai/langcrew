...
| 参数 | 类型 | 描述 | 默认值 |
|------|------|------|--------|
| `agents` | List[Agent] | 团队中的智能体列表 | 必需 |
| `tasks` | List[Task] | 要执行的任务列表 | 必需 |
| `process` | Process | 执行流程类型 | Process.sequential |
| `verbose` | bool | 启用详细日志记录 | False |
| `memory` | bool \| BaseMemory | 启用智能体间记忆 | False |
| `cache` | bool | 启用任务结果缓存 | True |
| `max_rpm` | int | 每分钟最大请求数 | None |
| `language` | str | 智能体通信语言 | "en" |
| `hitl` | bool \| HITLConfig | 人机交互 | None |
| `full_output` | bool | 返回完整执行结果 | False |
| `step_callback` | callable | 每步执行后的回调 | None |
| `task_callback` | callable | 每个任务完成后的回调 | None |
| `share_crew` | bool | 智能体间共享团队实例 | False |
| `planning` | bool | 启用自动任务规划 | False |
| `planning_llm` | BaseLLM | 规划专用的LLM | None |
=======
| 参数      | 类型        | 描述                    | 必需 |
| --------- | ----------- | ----------------------- | ---- |
...
<<<<<<< HEAD
- **[智能体概念](/zh/concepts/agents)** - 深入了解智能体设计
- **[任务概念](/zh/concepts/tasks)** - 学习任务配置和管理
- **[记忆系统](/zh/guides/memory/getting-started)** - 实现智能体记忆
- **[HITL指南](/zh/guides/hitl/getting-started)** - 集成人机协作
=======
