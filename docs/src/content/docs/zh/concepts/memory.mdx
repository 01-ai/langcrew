...
内存管理使用 LangGraph 的 Store 和 Checkpointer 系统为 AI 智能体工作流提供持久状态和上下文管理，使智能体能够跨会话维护对话历史、知识和实体信息。

## 什么是内存管理？

langcrew 中的内存管理提供：
- **短期记忆**：使用 LangGraph Checkpointer 的基于会话的对话历史
- **长期记忆**：使用 LangGraph Store 系统的持久知识存储
- **实体记忆**：跨对话跟踪和记住实体（人员、组织、概念）
- **多提供商支持**：内存、SQLite、PostgreSQL 后端（已测试）；Redis、MongoDB、MySQL（实验性）
