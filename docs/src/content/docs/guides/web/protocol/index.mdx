...
- `session_id` (optional): Session identifier for conversation continuity. If not provided or empty, a new session will be automatically created with format `session_{16-character-uuid}`
...
response = requests.post('/api/v1/chat', json={
    ...
    "session_id": "session_abc123def456"  # Use existing session
})
