...
const FileViewer = () => {
  ...
  const fileViewerRenderer = () => {
    ...
    // Append size= to bust caches after the model rewrites a file
    // const url = typeof size === 'number' ? appendQueryParam(fileUrl, 'size', size) : fileUrl;
    const url = fileUrl;

    return (
      <FileReader
        key={url} // Remount when the URL changes so stale content is not shown
        url={url}
    ...
    >
      <div className="h-14 px-4 flex items-center justify-between flex-shrink-0 border-b border-[#e9e9e9]">
        ...
        <div className="flex items-center justify-end gap-5 shrink-0">
          <button
            type="button"
            className="h-9 border border-[#d8d8d8] rounded-[8px] px-3 py-2 flex items-center gap-1 text-[14px] leading-5 text-black cursor-pointer"
            onClick={() => downloadAttachment(fileViewerFile)}
            aria-label={t('attachment.download')}
            title={t('attachment.download')}
          >
            <span className="w-4 h-4 flex items-center justify-center shrink-0">
              <FileViewerDownloadSvg className="block" aria-hidden="true" />
            </span>
          ...
          {!fileViewerMaximized && (
            ...
            >
              <FileViewerZoomOutSvg className="block" aria-hidden="true" />
            </button>
          )}
          ...
          >
            <FileViewerCloseSvg className="block" aria-hidden="true" />
          </button>
        </div>
      </div>
