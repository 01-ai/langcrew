...
const AgentHeader: FC<AgentHeaderProps> = ({ headerNode, backButtonNode, shareButtonNode }) => {
  // ✅ Ensure Hooks Always called at the top
  const { layoutConfig, sessionInfo } = useAgentStore();
  const storeApi = useAgentStoreApi();

  // ✅ All the logic and the condition are in the judgement. Hooks After
  const newChat = useCallback(() => {
    storeApi.getState().resetStore();
  }, [storeApi]);

  // ✅ Use useMemo To stabilize. headerNode Rendering results
  ...
  // ✅ If there is a custom header，Go straight back.
  if (customHeader) {
    return <>{customHeader}</>;
  }

  // ✅ embedded Mode (%1)narrowMode=true）None session Do not show time
