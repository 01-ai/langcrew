...
const AllFilesModal: React.FC<AllFilesModalProps> = ({ open, onClose, sessionId }) => {
  ...
  const handleBatchDownload = async () => {
    ...
    try {
      ...
      // Download every selected file and add it to the ZIP archive.
      ...
      // Do not show a success message when any file failed to download.
      if (hasFailedFiles) {
        hideLoading();
      ...
      // Use the current timestamp in the archive name.
      ...
      hideLoading();
      console.error('Failed to create the download archive:', error);
      message.error(t('files.batch.failed'));
    } finally {
      setIsDownloading(false);
    }
  ...
  return (
    ...
    >
      <Modal
        // Hide the default close button because the title renders its own.
        closable={false}
        styles={{
          mask: {
          ...
          header: {
            // Keep the custom title area at full width.
            width: 420,
            marginLeft: 20,
            paddingTop: 16,
            paddingBottom: 16,
            ...
            <div className="flex items-center justify-between border-gray-200">
              ...
              <div className="flex items-center gap-2">
                <Button
                  ...
                  // Use the flex gap instead of a right margin.
