...
import { MessageChunk, MessageToolChunk, PlanStep, TaskStatus } from '@/types';
      ...
      if (isToolMessage(item as MessageToolChunk)) {
        return (
          <div className="flex flex-col gap-2 w-full max-w-full">
            {item.content && <Markdown content={item.content} className="!text-[16px] !leading-[28px] !text-[#000]" />}
            <ToolRender message={item as MessageToolChunk} />
          </div>
        );
      }
      return <MessageBrief key={item.id} message={item} />;
    }
