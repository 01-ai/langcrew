...
const getFeatureText = (model?: ModelItem) => {
  ...
  if (model?.ext?.feature_en) {
    ...
    // Use feature for Chinese
    if (lang === 'zh') {
      return model?.ext?.feature;
    }
    // Fall back to English when the user language is unknown
...
const ModelSelector: React.FC<ModelSelectorProps> = ({
  ...
  const menuItems = useMemo(() => {
    ...
    const itemRender = (item: any) => {
      ...
      // Description truncation is handled with CSS line-clamp
  ...
  const renderTrigger = () => {
    ...
    // Selected state: colored chip + clear button
    if (hasSelection) {
      return (
    ...
    // Unselected state: round button aligned with the tools control
    return (
