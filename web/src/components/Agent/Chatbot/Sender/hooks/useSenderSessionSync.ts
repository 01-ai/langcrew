...
import type { KnowledgeBaseItem, MCPToolItem, ModelItem, SessionInfo } from '@/types';
...
interface UseSenderSessionSyncProps {
  ...
  setSelectedSenderMCPTools: (items: MCPToolItem[]) => void;
...
export const useSenderSessionSync = ({
  ...
  const updateSelectedItems = useCallback(
    (session: SessionInfo) => {
      ...
      const sessionPluginToolItems = sessionToolItems.filter(
        (item) => item.agent_tool_type === 'builtin' || item.agent_tool_type === 'API',
      );
      const sessionWorkflowToolItems = sessionToolItems.filter((item) => item.agent_tool_type === 'workflow');
      const sessionSkillToolItems = sessionToolItems.filter((item) => item.agent_tool_type === 'skills');

      if (sessionToolItems.length) {
        const selectedMCPTools =
          senderMCPTools.filter((item) => sessionMcpToolItems.some((tool) => tool.agent_tool_id === item.agent_tool_id)) || [];
        const selectedSandboxTools =
          senderSandboxTools.filter((item) => sessionAgentToolItems.some((tool) => tool.agent_tool_id === item.agent_tool_id)) ||
          [];
        const selectedPluginTools =
          senderPluginTools.filter((item) => sessionPluginToolItems.some((tool) => tool.agent_tool_id === item.agent_tool_id)) ||
          [];
        const selectedWorkflowTools =
          senderWorkflowTools.filter((item) =>
            sessionWorkflowToolItems.some((tool) => tool.agent_tool_id === item.agent_tool_id),
          ) || [];
        const selectedSkillTools =
          senderSkillTools.filter((item) => sessionSkillToolItems.some((tool) => tool.agent_tool_id === item.agent_tool_id)) || [];
        ...
        if (showSenderActions && menuItems.length) {
          const sessionToolIds = sessionToolItems.map((item) => item.agent_tool_id);
          const matchedScene = menuItems.find((item) => {
            if (!item.agent_tools?.length) return false;
            return item.agent_tools.every((toolId) => sessionToolIds.includes(toolId));
          });
          if (matchedScene) {
            setSelectedScene(matchedScene.key);
          }
