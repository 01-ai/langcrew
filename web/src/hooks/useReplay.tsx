...
const STREAMING_INTERVAL = 10; // Streaming playback interval
const buttonStyle = {
  width: 76,
  height: 36,
};
...
const useReplay = (replayId: string, needPassword: boolean, defaultPassword: string = '') => {
  ...
  // Password input ref for autoFocus
  ...
  // Extra state: whether the password must be re-entered
  ...
  const start = useCallback(() => {
    ...
    const playNextChunk = () => {
      if (index < originalChunks.current.length) {
        ...
        // Interval based on the next chunk type
        const nextInterval =
  ...
  // Fetch share details — no askPassword dependency
  const getShareChatDetail = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/v1/sessions/share/${replayId}?${new URLSearchParams(params).toString()}`, {
        method: 'GET',
        ...
        for await (const chunk of XStream({
          ...
          if (chunkData) {
            ...
            if (chunkData.type === 'password_error') {
              ...
              // Set re-enter-password instead of calling askPassword
  ...
  // Watch the re-enter-password flag
  useEffect(() => {
