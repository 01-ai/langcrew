...
const appendQueryParam = (url: string, key: string, value: string): string => {
  ...
  try {
    ...
    // Fall back for non-standard URLs such as relative paths.
...
const WebsiteDeliveryDetailRenderer: React.FC<DetailRendererProps> = ({ message }) => {
  ...
  const handleDownload = () => {
    ...
    if (!url) return;
    // Trigger a download. The browser may ignore the download attribute, so allow a new tab as a fallback.
  ...
  const handleRefresh = () => {
    // Refresh the preview iframe and reload the selected text file.
  ...
  const handleClose = () => {
    // Close the workspace and file viewer in the right panel.
  ...
  // Match the loading state used by other tools while execution is in progress.
  ...
  // Render only the container until a valid result is available.
  if (!data) {
    ...
    <div className={containerClassName}>
      ...
      <div className="flex justify-between items-center h-[56px] w-full px-[15px]">
        ...
        <div className="flex-1 h-[36px] flex items-center justify-end gap-[12px]">
          ...
          {/* Segmented control for preview and code views. */}
                    ...
                    <div>
                      <div>
                        <Tree.DirectoryTree
                          ...
                          className="agentx-website-delivery-tree"
                        />
                      </div>
                    </div>
                  </div>
