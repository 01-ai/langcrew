...
const MessageBrief = ({ message, session: sessionProp, ...props }: BriefRendererProps) => {
  const { stopped, sessionInfo } = useAgentStore();
  ...
  return <Brief message={message} send={send} stopped={stopped} session={sessionProp ?? sessionInfo} {...props} />;
};
