 ...
 * Format: chat-uid-{Timetamp36Evolution}-{Random string}
 * @param prefix Optional prefix, default to 'chat-uid'
...
/**
 * Use modern browserAPIGenerate safer ones.uid (Options)
 * @param prefix Optional prefix, default to 'chat-uid'
 * @returns Unique ID string
 */
export const generateSecureUid = (prefix: string = 'chat-uid'): string => {
  ...
  // Downgrade to original scheme
  return generateUid(prefix);
};

/**
 * Fetch file extensions with points
 * @param filename Filename
 * @returns Returns extension with point (e. g.)".txt"），Return empty string if no extension
 */
export function getFileExtensionFromFileName(filename: string): string {
  ...
  // Deal with similar".gitignore"hidden files
  if (lastDotIndex === 0) {
    return filename;
  }

  // Deal with similar"file."Situation
...
/**
 * File type description (used for error tips)
 */
export const ALLOWED_FILES_DESC = ALLOWED_FILES.replace(/\./g, '').split(',').join('/');

/**
 * Check if files are allowed for upload
 * @param filename Filename
 * @returns Allow Upload
 */
...
export const mergeFiles = (existingFiles: FileItem[], newFiles?: FileItem[]) => {
  ...
  // newFiles When there's a time, filter it out. existingFiles Medium .pptx Documentation
...
export const downloadAttachment = async (attachment: E2BFile | FileItem) => {
  ...
  try {
    ...
    // IffetchFailed. Back to original method.
    const a = document.createElement('a');
    a.href = url;
    a.download = filename;
    a.click();
