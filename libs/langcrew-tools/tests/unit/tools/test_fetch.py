...
class TestWebFetchTool:
    ...
    def test_default_configuration(self):
        ...
        assert tool.service_url == "http://localhost:11235"
        assert tool.llm_provider == "openai/gpt-4o-mini"
        assert tool.llm_api_key is None
    ...
    def test_init_with_constructor_params(self):
        ...
        assert tool.service_url == "https://custom.example.com"
        assert tool.llm_provider == "openai/gpt-4"
        assert tool.llm_api_key == "test-api-key"
        assert tool.proxy == "http://proxy.example.com:8080"

    def test_init_with_env_vars(self, monkeypatch):
        """Test initialization with environment variables."""
        monkeypatch.setenv("LANGCREW_WEB_FETCH_SERVICE_URL", "https://env.example.com")
        ...
        monkeypatch.setenv("LANGCREW_WEB_FETCH_LLM_PROVIDER", "openai/gpt-3.5-turbo")
        monkeypatch.setenv("LANGCREW_WEB_FETCH_LLM_API_KEY", "env-api-key")
        monkeypatch.setenv("LANGCREW_WEB_FETCH_PROXY", "http://env-proxy.com:3128")

        tool = WebFetchTool()
        assert tool.service_url == "https://env.example.com"
        assert tool.timeout == 90
        assert tool.max_content_length == 64000
        assert tool.filter_type == "pruning"
        assert tool.llm_provider == "openai/gpt-3.5-turbo"
        assert tool.llm_api_key == "env-api-key"
        assert tool.proxy == "http://env-proxy.com:3128"

    def test_llm_api_key_default(self):
        """Test that llm_api_key defaults to None when not configured."""
        tool = WebFetchTool()
        assert tool.llm_api_key is None
    ...
    def test_priority_order(self, monkeypatch):
        ...
        monkeypatch.setenv("LANGCREW_WEB_FETCH_SERVICE_URL", "https://env.example.com")
        monkeypatch.setenv("LANGCREW_WEB_FETCH_TIMEOUT", "90")
        monkeypatch.setenv("LANGCREW_WEB_FETCH_LLM_API_KEY", "env-key")
        monkeypatch.setenv("OPENAI_API_KEY", "openai-key")

        # Constructor params should override env vars
        tool = WebFetchTool(
            service_url="https://constructor.example.com",
            timeout=45,
            llm_api_key="constructor-key",
        )

        assert tool.service_url == "https://constructor.example.com"
        assert tool.timeout == 45
        assert tool.llm_api_key == "constructor-key"
    ...
    async def test_arun_success_pruning_filter(self):
        ...
        tool = WebFetchTool(service_url="https://api.example.com")
    ...
    async def test_arun_with_proxy(self):
        ...
        tool = WebFetchTool(
            service_url="https://api.example.com", proxy="http://proxy.example.com:8080"
        )
    ...
    async def test_arun_content_truncation(self):
        ...
        tool = WebFetchTool(
            service_url="https://api.example.com", max_content_length=20
        )
    ...
    async def test_arun_uses_instance_filter_type_when_none_provided(self):
        ...
        tool = WebFetchTool(
            service_url="https://api.example.com", filter_type="pruning"
        )
