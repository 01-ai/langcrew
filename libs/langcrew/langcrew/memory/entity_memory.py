...
class EntityMemory:
    ...
    def save(
        ...
        if existing:
            ...
            entity_record["created_at"] = existing.get(
                "created_at", entity_record["last_updated"]
            )
        ...
        # Save to store
        self.store.put(namespace=namespace, key=key, value=entity_record)
    ...
    def update_entity(
        self, entity_name: str, updates: dict[str, Any], entity_type: str | None = None
    ) -> bool:
        ...
        self.store.put(namespace=namespace, key=key, value=entity)
