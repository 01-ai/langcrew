...
from langcrew.tools.converter import (
    ToolConverter,
...
class TestToolConverterCrewAIConversion:
    ...
    def test_convert_crewai_tool_invalid_tool(self, invalid_tool):
        ...
        result = ToolConverter.convert_crewai_tool(invalid_tool)
...
class TestToolConverterCallableConversion:
    ...
    def test_convert_callable_tool_no_name_func(self):
        ...
        lambda_func = lambda x: f"Lambda result: {x}"
...
class TestToolConverterUniversal:
    ...
    def test_convert_tool_unknown_type(self):
        ...
        result = ToolConverter.convert_tool(unknown_obj)
...
class TestToolConverterUtilities:
    ...
    def test_create_args_schema_from_langchain_existing_schema(self, mock_langchain_tool):
...
class TestConvenienceFunctions:
    ...
    def test_convert_tools_mixed_types(self, mock_crewai_tool, mock_langchain_tool, sample_function):
    ...
    def test_convert_crewai_tools_batch(self, mock_crewai_tool, mock_crewai_tool_no_params):
    ...
    def test_convert_langchain_tools_batch(self, mock_langchain_tool, async_mock_langchain_tool):
    ...
    def test_create_crewai_tool_from_function_complex_params(self, complex_function):
        ...
        result = create_crewai_tool_from_function(
            complex_function,
            name="complex_tool",
            description="Complex function tool"
        )
...
class TestBidirectionalConversion:
    ...
    def test_crewai_to_langchain_to_crewai(self, mock_crewai_tool):
        ...
        original_result = mock_crewai_tool._run(query="test")
    ...
    def test_langchain_to_crewai_to_langchain(self, mock_langchain_tool):
        ...
        # Test that functionality is preserved
        original_result = mock_langchain_tool._run(query="test")
    ...
    def test_batch_mixed_conversion(self, mock_crewai_tool, mock_langchain_tool, sample_function):
        """Test batch conversion of mixed tool types."""
        # Convert all to LangChain format
        langchain_tools = convert_tools([mock_crewai_tool, mock_langchain_tool, sample_function])
...
class TestEdgeCases:
    ...
    def test_exception_propagation_and_logging(self, mock_crewai_tool):
        ...
        with patch('langcrew.tools.tool_converter.logger') as mock_logger:
            # Force an exception during conversion
            with patch.object(mock_crewai_tool, '_run', side_effect=Exception("Test exception")):
                result = ToolConverter.convert_crewai_tool(mock_crewai_tool)
                assert result is not None
    ...
    def test_convert_tool_with_invalid_inputs(self, invalid_input):
        ...
        result = ToolConverter.convert_tool(invalid_input)
