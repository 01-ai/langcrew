...
class TestAsyncStreaming:
    ...
    async def test_crew_astream_basic(self, mock_astream):
        ...
        async def async_generator():
            ...
            yield {"messages": [HumanMessage(content="Async hello"), AIMessage(content="Async response")]}
    ...
    async def test_crew_astream_error_handling(self):
        ...
        with patch.object(crew, 'astream', side_effect=Exception("Async error")):
            with pytest.raises(Exception, match="Async error"):
                async for chunk in crew.astream({"messages": [HumanMessage(content="Test")]}):
                    pass
...
class TestStreamEvents:
    ...
    async def test_crew_astream_events_basic(self, mock_astream_events):
        ...
        task = Task(
            ...
            expected_output="Event response",
            agent=agent
        )
    ...
    async def test_crew_astream_events_versions(self, mock_astream_events):
        ...
        task = Task(
            ...
            expected_output="Version response",
            agent=agent
        )
...
class TestStreamingConfiguration:
    ...
    async def test_astream_events_parameters(self):
        ...
        for params in test_params:
            with patch.object(crew, 'astream_events', return_value=async_generator_empty()) as mock_astream_events:
                events = []
                async for event in crew.astream_events(
...
class TestStreamingErrorHandling:
    ...
    def test_stream_connection_error(self):
        ...
        with patch.object(crew, 'stream', side_effect=ConnectionError("Connection failed")):
    ...
    async def test_astream_timeout_error(self):
        ...
        task = Task(
            ...
            expected_output="Timeout response",
            agent=agent
        )
    ...
    async def test_astream_events_invalid_parameters(self):
        ...
        task = Task(
            ...
            expected_output="Invalid response",
            agent=agent
        )
        ...
        with patch.object(crew, 'astream_events', side_effect=ValueError("Invalid version")):
